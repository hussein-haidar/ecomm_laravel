        ...
        <div class="col-lg-10">
            <div class="card border-0 shadow-sm" data-aos="fade-up">
                <div class="card-header bg-white border-0">
                    <h4 class="fw-bold mb-0"><i class="fas fa-shopping-cart me-2 text-primary"></i>Checkout Pesanan</h4>
                </div>
                <div class="card-body p-4 p-lg-5">
                    @if (!empty($keranjang_terpilih))
                        <form action="{{ route('pelanggan_data.saveBeli') }}" method="POST" enctype="multipart/form-data" novalidate>
                            @csrf
                            @if ($errors->any())
                                            ...
                                            <tr>
                                                <td>
                                                    @if (!empty($item['foto_produk']))
                                                        <img src="{{ asset('fotoproduk/' . $item['foto_produk']) }}" class="rounded" width="60" height="60" style="object-fit: cover;" alt="{{ $item['nama_produk'] }}">
                                                    @else
                                                        <span class="text-muted">Tidak ada foto</span>
                                                    @endif
                                                </td>
                                                <td class="fw-medium">{{ $item['nama_produk'] }}</td>
                                                <td><span class="badge bg-light text-dark border">{{ $item['ukuran_produk'] }}</span></td>
                                                <td class="text-center">{{ $item['jumlah_produk'] }} {{ $item['satuan_produk'] }}</td>
                                                <td class="text-end">Rp{{ number_format($item['harga_produk'], 0, ',', '.') }}</td>
                                                <td class="text-end fw-semibold text-success">Rp{{ number_format($item['harga_produk'] * $item['jumlah_produk'], 0, ',', '.') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            ...
                            <input type="hidden" id="latitude_pelanggan" name="latitude" value="{{ $latitude_pelanggan ?? '' }}" readonly>
                            <input type="hidden" id="longitude_pelanggan" name="longitude" value="{{ $longitude_pelanggan ?? '' }}" readonly>
                    ...
                    @else
                        <div class="alert alert-warning text-center py-5">
                            ...
                            <h5>Tidak ada produk yang dipilih</h5>
                            <a href="{{ route('pelanggan_data.cart') }}" class="btn btn-primary rounded-pill px-4 mt-3">
