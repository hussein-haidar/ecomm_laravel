...
<body>
    <div class="kop-surat">
        ...
        @endif
        <h1>{{ $nama_toko }}</h1>
        <h2>LAPORAN MUTASI PRODUK BULANAN</h2>
        <p>Alamat: {{ $alamat }}</p>
        <p>Telpon: {{ $telpon }}</p>
    </div>

    <div class="report-date">
        Laporan Mutasi Produk Dibuat: {{ \Carbon\Carbon::now()->locale('id')->translatedFormat('d F Y') }}
        @if ($start_month && $end_month)
            <br>Periode: {{ $start_month }} s/d {{ $end_month }}
        @endif
    </div>
